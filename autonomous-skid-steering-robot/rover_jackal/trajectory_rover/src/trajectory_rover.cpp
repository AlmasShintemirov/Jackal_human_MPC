...
TRAJECTORY::TRAJECTORY(double _dt, struct traj_input_struct_& _traj_input_struct)
{
    ...
    pos_ref_start_msg.pose.position.x = 25;
    pos_ref_start_msg.pose.position.y = 7;
    ...
    while (ros::ok() && _traj_input_struct.traj_on_switch)
    {
        ...
        if (_traj_input_struct.traj_num == 4)
        {
            ...
            pos_xyz[0] = pos_xyz_atTrajStart[0] -
                         _traj_input_struct.radius * cos(_traj_input_struct.rotvel * traj_time + M_PI / 2);
            pos_xyz[1] = pos_xyz_atTrajStart[1] +
...
void TRAJECTORY::setPositionToReference()
{
    pos_xyz[0] = pos_ref_start_msg.pose.position.x;
    pos_xyz[1] = pos_ref_start_msg.pose.position.y;
    pos_xyz[2] = pos_ref_start_msg.pose.position.z;
...
void TRAJECTORY::setReferencePositionToLast()
{
    pos_ref_start_msg.pose.position.x = pos_xyz_last[0];
    pos_ref_start_msg.pose.position.y = pos_xyz_last[1];
    pos_ref_start_msg.pose.position.z = pos_xyz_last[2];
    heading = heading_last;
}

std::vector<double> TRAJECTORY::getPosition()
{
    return pos_xyz;
}

double TRAJECTORY::getHeading()
{
    return heading;
}
